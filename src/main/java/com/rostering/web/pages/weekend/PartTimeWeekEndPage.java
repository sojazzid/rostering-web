...
import com.rostering.web.pages.weekend.modal.PartTimeWeekEndModalSave;
...
public class PartTimeWeekEndPage extends BasePage {
    ...
    private PartTimeWeekEndModalSave partTimeWeekEndModalSave;

    public PartTimeWeekEndPage() {

        formFilter = createForm("formFilter", new PartTimeWeekEndFormFilter());
        add(formFilter);
        ...
        try {
            loadMaquinistas(driverDao.getByType(ShiftType.PART_TIME_WEEKEND));
        } catch (PersistenceException e) {
            //TODO handle exception
            e.printStackTrace();
        }
        try {
            loadTurnos(shiftDao.getByType(ShiftType.PART_TIME_WEEKEND));
        } catch (PersistenceException e) {
            //TODO handle exception
            e.printStackTrace();
        }
        ...
        partTimeWeekEndModalSave = new PartTimeWeekEndModalSave("modalSave", "Nombre proceso asignación");
        step2.add(partTimeWeekEndModalSave);
    ...
    private AjaxButton createGuardarButton(String name) {
        AjaxButton button = new AjaxButton(name) {
            ...
            protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
                ...
                target.add(formFilter);
                target.add(partTimeWeekEndModalSave);
                target.add(asignacionGroup);

                List<Allocation> asignaciones = new ArrayList<Allocation>(asignacionDataView.getModelObject());

                partTimeWeekEndModalSave.setAsignaciones(asignaciones);
                partTimeWeekEndModalSave.setFeedbackPanel(feedbackPanel);
                partTimeWeekEndModalSave.show(true);
    ...
    protected String getPageTitle() {
        return "Asignar Maquinistas Part-time Fin de Semana";
    }

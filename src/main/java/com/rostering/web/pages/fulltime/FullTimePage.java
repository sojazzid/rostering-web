...
public class FullTimePage extends BasePage {
    ...
    private ModalSave modalSave;

    public FullTimePage() {

        formFilter = createForm("formFilter", new FullTimeFormFilter());
        add(formFilter);
        ...
        try {
            loadMaquinistas(driverDao.getByType(ShiftType.FULL_TIME));
        } catch (PersistenceException e) {
            //TODO handle exception
            e.printStackTrace();
        }
        try {
            loadTurnos(shiftDao.getByType(ShiftType.FULL_TIME));
        } catch (PersistenceException e) {
            //TODO handle exception
            e.printStackTrace();
        }
        ...
        modalSave = new ModalSave("modalSave", "Nombre proceso asignación");
        step2.add(modalSave);
    ...
    private AjaxButton createGuardarButton(String name) {
        AjaxButton button = new AjaxButton(name) {
            ...
            protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
                ...
                target.add(formFilter);
                target.add(modalSave);
                target.add(asignacionGroup);

                List<Allocation> asignaciones = new ArrayList<Allocation>(asignacionDataView.getModelObject());

                modalSave.setAsignaciones(asignaciones);
                modalSave.setFeedbackPanel(feedbackPanel);
                modalSave.show(true);
    ...
    protected String getPageTitle() {
        return "Asignar Maquinistas Full-time";
    }
